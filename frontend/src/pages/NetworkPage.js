...
const D = { hero:{overline:'',title:'',body:''}, stats:[], domains:[], cta:{title:'',body:''} };
...
export default function NetworkPage() {
  ...
  return (
    <div data-testid="network-page">
      ...
      {ragers.length > 0 && (
        <section className="py-20 border-b border-gray-100"><div className="max-w-[1400px] mx-auto px-6"><p className="rage-overline mb-4">The Ragers</p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-gray-200 mt-8">{ragers.map(m=>(<div key={m.id} className="bg-white p-8">
            {m.photo_url ? (
              <img src={m.photo_url} alt={m.name} className="w-12 h-12 object-cover rounded-full mb-4" />
            ) : (
              <div className="w-10 h-10 bg-[#DC143C]/10 flex items-center justify-center mb-4"><span className="text-[#DC143C] text-sm font-semibold font-mono">{m.name.split(' ').map(n=>n[0]).join('')}</span></div>
            )}
            ...
            {m.categories?.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-3">{m.categories.map(cat => <span key={cat} className="text-[10px] uppercase tracking-wider bg-gray-100 text-gray-500 px-2 py-0.5">{cat}</span>)}</div>
            )}
          </div>))}</div>
        </div></section>
      )}
